(() => {
  ...
  button.title = "ConPin";
  button.setAttribute("aria-label", "Open ConPin");
  ...
  panel.setAttribute("aria-label", "ConPin");
  ...
  setTimeout(() => {
    if (!rpc) {
      clearInterval(connect);
      error(
        "ConPin bridge unavailable. Run “ConPin: Install / Repair Codex Button” and reload VS Code.",
      );
    }
  }, 15000);
  setMode(advanced);
