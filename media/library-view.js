(() => {
  ...
  function ruleRow(rule, state, focus) {
    ...
    return `<article class="rule-row ${focus === rule.key ? "focused" : ""}"><input type="checkbox" data-select="${esc(rule.key)}" aria-label="Select ${esc(rule.title)}" ${state.wanted.includes(rule.key) ? "checked" : ""} ${blocked ? "disabled" : ""}><button class="rule-link" data-rule="${esc(rule.key)}"><strong>${esc(rule.title)}</strong><span class="muted">${esc(rule.category)}${rule.appliesTo && rule.appliesTo !== "." ? ` · ${esc(rule.appliesTo)}/` : ""}</span><span class="badges">${rule.isDefault ? '<span class="badge">Default</span>' : ""}${rule.disabledHere ? '<span class="badge">Disabled here</span>' : rule.overriddenBy ? '<span class="badge">Overridden here</span>' : ""}</span></button></article>`;
  }
  function list(state, focus, query, category, globalOpen, selectedOnly = false) {
    const matches = r => (!selectedOnly || state.selected.includes(r.key)) && (!category || r.category === category) && `${r.title} ${r.text} ${r.category}`.toLowerCase().includes(query.toLowerCase());
    ...
    return `<section><div class="section-heading"><div><h2>${esc(projectName)}</h2><p class="muted">Select with the checkbox. Open a title to read or edit.</p></div><span class="count">${project.length} rules</span></div>${project.map(r=>ruleRow(r,state,focus)).join("") || `<div class="empty"><h3>${state.project ? "Build this project's rule set" : "Open a project folder"}</h3><p>${query || category || selectedOnly ? "No rules match your filters." : "Add a rule or generate suggestions from project documents."}</p>${query || category || selectedOnly ? button("reset-filters","Reset filters") : state.project ? button("generate","Generate rules from project") : ""}</div>`}</section><details class="global-section" ${globalOpen || query || category || selectedOnly ? "open" : ""}><summary>Global rules <span class="count">${state.selected.filter(k=>state.rules.some(r=>r.key===k && r.scope === "personal")).length} active · ${global.length} shown</span></summary><p class="muted">Available in every project. Disable or override a rule here without changing other projects.</p>${global.map(r=>ruleRow(r,state,focus)).join("") || '<p class="empty">No matching global rules.</p>'}${button("new-global","+ Global rule")}</details>`;
  }
  ...
  function inspector(state, focus, graph = false) {
    ...
    return `<div class="section-heading"><span class="eyebrow">${r.scope === "project" ? "Project rule" : "Global rule"}</span>${button("unfocus","← Back to rules",'aria-label="Back to rules"')}</div><h2>${esc(r.title)}</h2><div class="badges">${badges(r,state)}</div><p class="rule-body">${esc(r.text)}</p>${r.appliesTo && r.appliesTo !== "." ? `<p class="muted">Applies only within <strong>${esc(r.appliesTo)}/</strong>.</p>` : ""}${override ? `<p class="notice">Replaced by <button data-rule="${esc(override.key)}">${esc(override.title)}</button> while that project rule is selected.</p>` : ""}<div class="inspector-actions">${selection}${button("edit","Edit saved rule",writable ? "" : "disabled")}</div><details class="detail-section" data-detail-section="defaults"><summary>Defaults & availability</summary><div class="inspector-actions">${button("default",r.scope === "personal" ? (r.globalDefault ? "Remove global default" : "Make global default") : (r.isDefault ? "Remove default" : "Make default"),writable ? "" : "disabled")}${r.scope === "personal" && state.project ? button("disable-global",r.disabledHere ? "Enable for this project" : "Disable for this project") : ""}</div><p class="muted">${r.scope === "personal" ? "Global defaults apply across projects unless disabled or overridden here." : "Defaults belong to this project."} Use Restore defaults to apply them to the current selection.</p></details>${r.sources?.length ? `<details class="detail-section" data-detail-section="sources"><summary>Sources · ${r.sources.length}</summary><div class="source-list">${r.sources.map((s,i)=>r.scope === "project" ? `<button data-source="${i}" data-source-rule="${esc(r.key)}">${esc(s.path)}:${s.line}</button>` : `<span>${esc(s.path)}:${s.line}</span>`).join("")}</div></details>` : ""}<details class="detail-section" data-detail-section="relationships"><summary>Relationships · ${links(state,r).length}</summary>${links(state,r).map(e=>{
    ...
    }).join("") || '<p class="muted">No relationships.</p>'}<div class="inspector-actions">${state.project ? `${r.scope === "project" ? button("override","Override a global rule") : ""}${button("conflict","Mark a conflict")}` : ""}${button("relationships","View relationships")}</div></details><details class="detail-section" data-detail-section="presets"><summary>Used in presets</summary>${state.presets.filter(p=>p.scope===r.scope && p.ruleIds.includes(r.id)).map(p=>`<button data-preset="${esc(p.key)}">${esc(p.title)}</button>`).join("") || '<p class="muted">No preset memberships.</p>'}</details>`;
  }
